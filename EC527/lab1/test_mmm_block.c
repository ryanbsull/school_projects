...
int main(int argc, char *argv[])
{
  ...
  printf("row_len, ijk, kij, jki, bijk\n");
  {
    int i, j;
...
void mmm_bijk(matrix_ptr a, matrix_ptr b, matrix_ptr c)
{
  long int block_sz = 64;
  long int i, j, k, jj, kk;
  long int length = get_matrix_row_length(a);
  long int blks = block_sz*(length / block_sz);
  data_t *a0 = get_matrix_start(a);
  data_t *b0 = get_matrix_start(b);
  data_t *c0 = get_matrix_start(c);
  data_t sum;
  
  for(kk = 0; kk < blks; kk += block_sz){
    for(jj = 0; jj < blks; jj += block_sz){
      for(i = 0; i < length; i++){
        for(j = jj; j < jj + block_sz; j++){
          sum = c0[i*length+j];
