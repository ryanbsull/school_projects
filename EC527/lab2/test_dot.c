...
#define A   30   /* coefficient of x^2 */
#define B   170   /* coefficient of x */
...
void dot1(array_ptr v0, array_ptr v1, data_t *dest);
void dot2(array_ptr v0, array_ptr v1, data_t *dest);
void dot3(array_ptr v0, array_ptr v1, data_t *dest);
...
int main(int argc, char *argv[])
{
  ...
  printf("Vector dot product examples\n");
  ...
  /* output times */
  printf("%s", "size, dot_unoptimized, dot_optimized2, dot_optimized6, dot_optimized10\n");
  {
    int i, j;
...
/*************************************************/
/* Dot1: basic implementation of array dot product */
void dot1(array_ptr v0, array_ptr v1, data_t *dest) {
  long int i;
  long int length = get_array_length(v0);
  data_t *data = get_array_start(v0);
  data_t *data1 = get_array_start(v1);
  data_t acc = IDENT;

  for (i = 0; i < length; i++) {
    acc = acc + (data[i] * data1[i]);
  }
  *dest = acc;
}
...
void dot2(array_ptr v0, array_ptr v1, data_t *dest) {
  ...
  for (i = 0; i < limit; i+=2) {
    acc0 = acc0 + (data[i] * data1[i]);
    acc1 = acc1 + (data[i+1] * data1[i+1]);
  }
  for(; i < length; i++){
    acc0 = acc0 + (data[i] * data1[i]);
  }
...
void dot3(array_ptr v0, array_ptr v1, data_t *dest) {
  ...
  for (i = 0; i < limit; i+=6) {
    acc0 = acc0 + (data[i] * data1[i]);
    acc1 = acc1 + (data[i+1] * data1[i+1]);
    acc2 = acc2 + (data[i+2] * data1[i+2]);
    acc3 = acc3 + (data[i+3] * data1[i+3]);
    acc4 = acc4 + (data[i+4] * data1[i+4]);
    acc5 = acc5 + (data[i+5] * data1[i+5]);
  }
  for(; i < length; i++){
    acc0 = acc0 + (data[i] * data1[i]);
  }
...
void dot4(array_ptr v0, array_ptr v1, data_t *dest) {
  ...
  for (i = 0; i < limit; i+=10) {
    ...
    acc4 = acc4 + (data[i+4] * data1[i+4]);
    acc5 = acc5 + (data[i+5] * data1[i+5]);
    acc6 = acc6 + (data[i+6] * data1[i+6]);
    acc7 = acc7 + (data[i+7] * data1[i+7]);
    acc8 = acc8 + (data[i+8] * data1[i+8]);
    acc9 = acc9 + (data[i+9] * data1[i+9]);
  }
  for(; i < length; i++){
    acc0 = acc0 + (data[i] * data1[i]);
  }
