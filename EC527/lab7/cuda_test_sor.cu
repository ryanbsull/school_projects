...
__global__ void kernel_sor(int rowlen, float* d_x){
  const int blk_sz = (int) (rowlen / blockDim.x);
  const int idx = blk_sz * threadIdx.x;
  const int off = blk_sz * threadIdx.y; 
  int i, j, it;
  double change;

  for(it = 0; it < ITERATIONS; it++){
    for(i = idx; i < idx+blk_sz; i++) { 
      for(j = off; j < off+blk_sz; j++) {
...
int main(int argc, char **argv){
  ...
  // Launch the kernel
  kernel_sor<<<NUM_BLOCKS, threads>>>(rowlen, d_x);
