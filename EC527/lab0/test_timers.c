...
int main(int argc, char *argv[])
{
  ...
  /* Test gettimeofday(). We try several powers of 10 as the number of
     ...
     after the delay loop. Find the difference in the usecond part of the
     struct and save.
     NOTE: there is a potential bug here: If we roll over seconds (high
     order part of struct) the microsecond count would be wrong. We fix
     that with a call to "usecs_of_timeval()". */

  printf("Using gettimeofday: \n");

  j = 0;
  for (limit = 100000000; limit >= 1; limit /= 10) {
    gettimeofday(&tv_start, NULL);
    for (i = 0; i <= limit; i++) steps = steps + 3;
  ...
  for (limit = 100000000; limit >= 1; limit /= 10) {
    sec0 = get_seconds();
    for (i = 0; i <= limit; i++) steps = steps + 2;
