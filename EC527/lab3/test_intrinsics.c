...
void element_wise_multiply(data_t* pA1, data_t* pA2, data_t* pR, long int nSize);
void element_wise_add(data_t* pA1, data_t* pA2, data_t* pR, long int nSize);
data_t SSE_dotproduct(data_t* pA1, data_t* pA2, long int nSize);
...
int main(int argc, char *argv[])
{
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    for (long k=0; k<OUTER_LOOPS; k++) {
      element_wise_add(pArray1, pArray2, pResult, n);
    }
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  }
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
    for (long k=0; k<OUTER_LOOPS; k++) {
      element_wise_multiply(pArray1, pArray2, pResult, n);
    }
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  }
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    ...
    data_t result;
    for (long k=0; k<OUTER_LOOPS; k++) {
      result = SSE_dotproduct(pArray1, pArray2, n);
    }
    printf("%f\n", result);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
  ...
  /* output times */
  printf("size, Scalar, vecSSE, vecAVX, Element-wise Add, Element-wise Multiply, vecSSE_dp\n");
  {
    for (int i = 0; i < x; i++) {
...
// Non Vectorized
void element_wise_add(data_t* pArray1, data_t* pArray2, data_t* pResult, long int nSize){
...
void element_wise_multiply(data_t* pArray1, data_t* pArray2, data_t* pResult, long int nSize){
...
data_t SSE_dotproduct(data_t* pArray1,       // [in] 1st source array
                data_t* pArray2,       // [in] 2nd source array
                long int nSize)            // [in] size of all
{
  long nLoop = nSize/4;
  data_t result[4];
  __m128   m1, accum;
  ...
  for (long i = 0; i < nLoop; i++){
    ...
    accum = _mm_add_ps(m1, accum);
    pSrc1++;
    pSrc2++;
  }
  
  /* sum all integers within vector found at : https://community.intel.com/t5/Intel-C-Compiler/What-s-the-best-way-to-sum-up-values-in-m128/td-p/803575 */  
  accum = _mm_hadd_ps(accum,accum);
  accum = _mm_hadd_ps(accum,accum);
  _mm_store_ps(result, accum);
  return result[0];
}
